...
function getTotalPatients()
{
	...
	$stmt = $pdo->prepare("SELECT COUNT(patient_id) as total_patients FROM patient");
	$stmt->execute();
	return $stmt->fetch(PDO::FETCH_ASSOC)['total_patients'];
}

// Function to get patients with pagination
function getPatientsWithPagination($limit, $offset)
{
	global $pdo;

	$stmt = $pdo->prepare("SELECT * FROM patient LIMIT :limit OFFSET :offset");
...
$patients = getPatientsWithPagination($records_per_page, $offset);
?>
...
<div class="flex items-center justify-center min-h-screen">
	...
	<h2 class="mb-4 text-2xl font-bold">All Registered Patients</h2>
	<?php if (empty($patients)) : ?>
	    <div class="text-red-500 font-bold mb-4">No patients found.</div>
	<?php else : ?>
		...
		<table class="table-auto w-full border-collapse">
			...
			<tr>
			    ...
			    <th class="px-4 py-2">Social Security Number</th>
			</tr>
		    </thead>
		    <tbody>
			<?php foreach ($patients as $patient) : ?>
				...
				<td class="border px-4 py-2"><?php echo $patient['social_security_number']; ?></td>
				<td class="border px-4 py-2"><a class="text-blue-500 hover:underline" href="../profiles/patient_profile.php?patient_id=<?php echo $patient['patient_id']; ?>">View Profile</a></td>
			    </tr>
			<?php endforeach; ?>
		    </tbody>
		</table>
