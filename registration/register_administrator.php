...
// Function to handle registration and insert administrator into the database
function registerAdministrator($name, $email, $mobile_number, $password)
{
    global $pdo;
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO administrator (name, email, mobile_number, password_hash) VALUES (?, ?, ?, ?)");
        $stmt->execute([$name, $email, $mobile_number, $hashedPassword]);
        return true;
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($name) || empty($email) || empty($mobile_number) || empty($password)) {
        ...
        if (registerAdministrator($name, $email, $mobile_number, $password)) {
...
<div class="flex items-center justify-center min-h-screen">
    <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        <h2 class="mb-4 text-2xl font-bold">Register Administrator</h2>
        ...
        <form method="POST" action="<?php echo $_SERVER["PHP_SELF"]; ?>">
            ...
            <div class="mb-4">
                ...
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="mobile_number" type="text" name="mobile_number" placeholder="Enter your mobile number">
            </div>
            <div class="mb-4">
